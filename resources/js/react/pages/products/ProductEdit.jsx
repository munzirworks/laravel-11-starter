  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (err.response?.status === 422) {
        ...
        setError('Unable to update product right now. Please try again.')
      }
    } finally {
      setSubmitting(false)
    }
    ...
    <DashboardLayout>
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-black/60">Update details in the shared product catalog.</p>
        </div>

        <Link to={`/products/${id}`} className="rounded-2xl px-3 py-1.5 text-sm text-black/80 ring-1 ring-black/10 transition hover:bg-black/5">
          Back to Detail
        </Link>
      </div>
      ...
      {forbidden && (
        <div className="rounded-3xl border border-black/5 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-black/60">Contact your administrator to request product update access.</p>
        </div>
      )}

      {!forbidden && loading && <div className="rounded-3xl border border-black/5 bg-white p-6 text-sm text-black/60 shadow-sm">Loading product data...</div>}
        ...
        <div className="rounded-3xl border border-black/5 bg-white p-6 shadow-sm sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              <Link to={`/products/${id}`} className="rounded-2xl px-4 py-2 text-sm font-medium text-black/80 ring-1 ring-black/10 transition hover:bg-black/5">
                Cancel
              </Link>
            </div>
          </form>
